				...
				<li class="fieldcontain">
					<span id="filter-label" class="property-label"><g:message code="filtercategorization.filter.label" default="Filter" /></span>
						...
						<span class="property-value" aria-labelledby="filter-label">
							<tmpl:/chooseEntityIconCRUD entity="${filtercategorizationInstance.filter}" />
							<g:link controller="filter" action="show" id="${filtercategorizationInstance?.filter?.id}">${filtercategorizationInstance?.filter?.id?.encodeAsHTML()} - ${filtercategorizationInstance?.filter?.servicedomain?.encodeAsHTML()}</g:link>
						</span>
