...
class JsecTagLib {
    ...
    def principal = { attrs ->
        ...
        if (subject != null) {
            ...
            if (attrs["type"]) {
                ...
                // requested.
                principal = subject.getPrincipals().oneByType(attrs["type"])
            }
            else {
    ...
    private boolean checkPermission(attrs, tagname) {
        ...
        if (!permClass) {
            ...
            if (!(permission instanceof org.jsecurity.authz.Permission) && !(permission instanceof String)) {
                throwTagError("Attribute [permission] must be a String or an instance of org.jsecurity.authz.Permission.")
            }
        }
        else {
